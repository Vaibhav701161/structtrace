...
function ResultContent({ result }: { result: RunResult }) {
  ...
  const deploymentEffect = summary.paired.difference_pp;
  const deploymentInterval = summary.bootstrap;
  const semanticEffect = summary.jointly_scored_semantic.paired.difference_pp;
  const regressionHotspots = aggregateHotspots(summary.primary_field_hotspots);
  const description = independentTotal === 0
    ? "No independent evidence units are available. Effect and interval estimates are intentionally withheld rather than displayed as zero."
  ...
  return (
    <div className="page page-wide result-page">
      ...
      <section className="evidence-context-bar" aria-label="Decision denominator and integrity"><span><b>{capturedTotal}</b> captured rows</span><span><b>{independentTotal}</b> independent units</span><span><b>{semanticTotal}</b> fully evaluated semantic pairs</span><span><ShieldCheck size={14} /> Freshly verified run view</span></section>
      ...
      <Card className="outcome-visualization">
        <div className="panel-heading"><div><h2>Deployment outcome distribution</h2><p>Pass and failure counts over {independentTotal} independent evidence units. The scale is shared.</p></div><Status tone={deploymentEffect == null ? "neutral" : deploymentEffect >= 0 ? "pass" : "fail"} label={formatEffect(deploymentEffect)} /></div>
      ...
      <Card className="metrics-card">
        ...
        <table className="metric-table"><caption>Metrics over independent, non-conflicting evidence units</caption><thead><tr><th>Metric</th><th>Baseline</th><th>Candidate</th><th>Change</th></tr></thead><tbody>{metricRows.map(([label, baseline, candidate]) => { const delta = independentTotal ? ((candidate - baseline) / independentTotal) * 100 : null; const reverse = label === "Valid but wrong"; const good = delta != null && (reverse ? delta <= 0 : delta >= 0); return <tr key={label}><th scope="row">{label}</th><td>{countPercent(baseline, independentTotal)}</td><td>{countPercent(candidate, independentTotal)}</td><td>{delta == null ? <span className="metric-na">N/A</span> : <span className={`delta ${good ? "delta-good" : "delta-bad"}`}>{delta >= 0 ? <ArrowUpRight size={15} /> : <ArrowDownRight size={15} />}{delta >= 0 ? "+" : ""}{delta.toFixed(1)} pp</span>}</td></tr>; })}</tbody></table>
      </Card>
      <Card className="metrics-card">
        ...
        <div className="stat-line"><span>Semantic-only candidate − baseline</span><strong>{formatEffect(semanticEffect)}</strong><small>{semanticEffect == null ? "Not estimable because no pair had explicit binary primary outcomes for both variants." : `${semanticTotal} pairs had explicit binary semantic outcomes for both variants. Operational failures are not relabelled as wrong answers.`}</small></div>
      </Card>
      <div className="result-grid">
        <Card className="transition-card">
          <div className="panel-heading"><div><h2>Independent deployment outcomes</h2><p>{independentTotal} non-conflicting evidence units contribute once each.</p></div><Button variant="ghost" icon={ListFilter} onClick={() => void navigate({ to: "/runs/$runId/cases", params: { runId: result.runId }, search: { search: "", focus: "" } })}>Inspect cases</Button></div>
          ...
          <div className="stat-line"><span>Candidate − baseline</span><strong>{formatEffect(deploymentEffect)}</strong><small>{deploymentInterval ? `95% paired interval [${deploymentInterval.lower_pp.toFixed(1)}, ${deploymentInterval.upper_pp.toFixed(1)}] pp` : "Interval not available: no independent evidence units."}</small></div>
          {deploymentEffect != null && deploymentInterval ? <EffectInterval estimate={deploymentEffect} lower={deploymentInterval.lower_pp} upper={deploymentInterval.upper_pp} /> : <div className="effect-unavailable"><strong>Effect not estimable</strong><span>No independent evidence units</span></div>}
        </Card>
        <Card className="hotspots-card">
          ...
          {regressionHotspots.length ? <div className="hotspot-list">{regressionHotspots.slice(0, 6).map((hotspot) => <button key={hotspot.pointer} onClick={() => void navigate({ to: "/runs/$runId/cases", params: { runId: result.runId }, search: { search: hotspot.pointer, focus: hotspot.pointer } })}><code>{hotspot.pointer}</code><span><i style={{ width: `${Math.max(4, hotspot.regressions / Math.max(1, regressions) * 100)}%` }} /></span><strong>{hotspot.regressions}</strong></button>)}</div> : <div className="no-hotspots"><CheckCircle2 size={20} /><strong>No field-level regressions recorded</strong><p>Open case evidence for complete transition details.</p></div>}
        </Card>
      </div>
      ...
      <div className="result-actions"><Button icon={XCircle} variant="secondary" disabled={result.integrity.status !== "verified"} onClick={() => void navigate({ to: "/runs/$runId/cases", params: { runId: result.runId }, search: { search: "", focus: "" } })}>Inspect regressions</Button><Button icon={Pin} variant="secondary" onClick={() => void navigate({ to: "/regressions" })}>Saved cases</Button>{result.integrity.status === "verified" && result.projectId && summary.gate.deployment_authorized && !accepted.isSuccess && <Button icon={ArrowRight} onClick={() => accepted.mutate()} disabled={accepted.isPending}>{accepted.isPending ? "Promoting…" : "Accept as next baseline"}</Button>}{accepted.data && <Button icon={ArrowRight} onClick={() => { void startNextIteration({ ...accepted.data.source, kind: "baseline", status: "ready" }).then(() => navigate({ to: "/new/source" })); }}>Open next comparison</Button>}</div>
...
function DistributionBar({ label, pass, total }: { label: string; pass: number; total: number }) {
  const percent = total ? pass / total * 100 : null;
  return <div className="distribution-row"><strong>{label}</strong><div className={`distribution-track ${percent == null ? "distribution-na" : ""}`} role="img" aria-label={percent == null ? `${label}: not estimable because there are no independent evidence units` : `${label}: ${pass} of ${total} deployment successes`}>{percent == null ? <span>Not estimable</span> : <><span className="distribution-pass" style={{ width: `${percent}%` }} /><span className="distribution-fail" style={{ width: `${100 - percent}%` }} /></>}</div><code>{pass}/{total}</code><b>{percent == null ? "N/A" : `${percent.toFixed(1)}%`}</b></div>;
}
...
export function formatEffect(value: number | null) { return value == null ? "Not estimable" : `${value >= 0 ? "+" : ""}${value.toFixed(1)} pp`; }
