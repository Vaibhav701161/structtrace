...
function RegressionsPage() {
  ...
  return <div className="page page-wide"><PageHeader title="Saved cases" description="A recurring regression suite: pinned case IDs are reevaluated on every candidate run and enforced before release promotion and in generated CI." />{pins.data?.length ? <Card className="pinned-list saved-case-list">{pins.data.map((pin) => <div key={pin.id}><span><Pin size={16} /><span><strong>{pin.caseId}</strong><small>{pin.projectName} · origin {pin.runId}{pin.lastRunId ? ` · last checked ${pin.lastRunId}` : ""}</small><input defaultValue={pin.note} placeholder="Add review note" aria-label={`Note for ${pin.caseId}`} onBlur={(event) => update.mutate({ id: pin.id, note: event.target.value, status: pin.status })} /></span></span><div><Status tone={pin.suiteStatus === "passing" || pin.suiteStatus === "fixed" ? "pass" : "fail"} label={pin.suiteStatus.replaceAll("_", " ")} /><select value={pin.status} aria-label={`Review status for ${pin.caseId}`} onChange={(event) => update.mutate({ id: pin.id, note: pin.note, status: event.target.value as "open" | "fixed" })}><option value="open">Review open</option><option value="fixed">Reviewed</option></select><Button variant="ghost" onClick={() => void navigate({ to: "/runs/$runId/cases/$caseId", params: { runId: pin.lastRunId ?? pin.runId, caseId: pin.caseId } })}>Open latest evidence</Button><button className="icon-button" aria-label={`Remove ${pin.caseId}`} onClick={() => remove.mutate(pin.id)}><Trash2 size={16} /></button></div></div>)}</Card> : <Card><EmptyState icon={Pin} title={pins.isLoading ? "Loading pinned suite…" : "No pinned cases"} description="Open a completed comparison and pin a case to enforce it on future candidates." /></Card>}</div>;
}
