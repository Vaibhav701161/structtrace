...
export function Cases() {
  ...
  const routeSearch = useSearch({ strict: false }) as { search?: string; focus?: string };
  ...
  return <CaseExplorer result={result} selectedId={caseId} initialSearch={routeSearch.search ?? ""} focusedPointer={routeSearch.focus ?? ""} />;
}

function CaseExplorer({ result, selectedId, initialSearch, focusedPointer }: { result: RunResult; selectedId?: string; initialSearch: string; focusedPointer: string }) {
  ...
  const open = (record: CaseRecord) => void navigate({ to: "/runs/$runId/cases/$caseId", params: { runId: result.runId, caseId: String(record.case?.id) }, search: { search, focus: focusedPointer } });
  const openedFocus = useRef(false);
  useEffect(() => {
    if (!focusedPointer || selectedId || openedFocus.current || records.length === 0) return;
    openedFocus.current = true;
    void navigate({ to: "/runs/$runId/cases/$caseId", params: { runId: result.runId, caseId: String(records[0].case?.id) }, search: { search, focus: focusedPointer } });
  }, [focusedPointer, navigate, records, result.runId, search, selectedId]);
  return (
    <div className="page page-full cases-page">
      ...
      <Card className="case-table-card">
        <div className="case-table-head"><span>Case ID</span><span>Transition</span><span>Primary finding</span><span>Baseline</span><span>Candidate</span><span>Structure</span></div>
      ...
      {selected && <CaseDrawer result={result} record={selected} focusedPointer={focusedPointer} close={() => void navigate({ to: "/runs/$runId/cases", params: { runId: result.runId }, search: { search, focus: "" } })} previous={selectedIndex > 0 ? () => open(records[selectedIndex - 1]) : undefined} next={selectedIndex >= 0 && selectedIndex + 1 < records.length ? () => open(records[selectedIndex + 1]) : undefined} />}
    </div>
  );
}
...
function OutcomeCell({ evaluation }: { evaluation: CaseRecord | undefined }) {
  const state = primaryOutcomeState(evaluation);
  const primaryIds = new Set(evaluation?.primary_outcome?.evaluator_ids ?? []);
  const diagnosticErrors = Object.values(evaluation?.evaluators ?? {}).filter((item: any) => item?.status === "error" && !primaryIds.has(String(item?.evaluator_id))).length;
  return <span className={`outcome-state outcome-${state.kind}`}>{state.kind === "pass" ? <Check size={14} /> : state.kind === "fail" ? <X size={14} /> : null}{state.label}{diagnosticErrors > 0 && <i title={`${diagnosticErrors} diagnostic evaluator error${diagnosticErrors === 1 ? "" : "s"}; primary outcome unchanged`}>D!</i>}</span>;
}
export function primaryOutcomeState(evaluation: CaseRecord | undefined): { label: string; kind: "pass" | "fail" | "error" | "na" | "unscored" | "incomplete" } {
  const primary = evaluation?.primary_outcome;
  if (!primary) return evaluation?.primary_pass === true ? { label: "Correct", kind: "pass" } : evaluation?.primary_pass === false ? { label: "Wrong", kind: "fail" } : { label: "Unscored", kind: "unscored" };
  if (primary.truth === "true" && primary.fully_evaluated) return { label: "Correct", kind: "pass" };
  if (primary.truth === "false" && primary.fully_evaluated) return { label: "Wrong", kind: "fail" };
  if (primary.truth === "error") return { label: "Evaluation error", kind: "error" };
  if (primary.truth === "not_applicable") return { label: "N/A", kind: "na" };
  if (primary.truth === "unscored") return { label: "Unscored", kind: "unscored" };
  return { label: "Incomplete", kind: "incomplete" };
}
...
function CaseDrawer({ result, record, focusedPointer, close, previous, next }: { result: RunResult; record: CaseRecord; focusedPointer: string; close: () => void; previous?: () => void; next?: () => void }) {
  ...
  const [changedOnly, setChangedOnly] = useState(record.deployment_transition === "baseline_only_pass" && !focusedPointer);
  ...
  const failingPointers = useMemo(() => {
    const primaryIds = new Set(record.candidate_evaluation?.primary_outcome?.evaluator_ids ?? []);
    const evaluators = Object.values(record.candidate_evaluation?.evaluators ?? {}).filter((item: any) => primaryIds.has(String(item?.evaluator_id))) as CaseRecord[];
  ...
  const diagnosticErrors = useMemo(() => {
    const primaryIds = new Set(record.candidate_evaluation?.primary_outcome?.evaluator_ids ?? []);
    return Object.values(record.candidate_evaluation?.evaluators ?? {}).filter((item: any) => item?.status === "error" && !primaryIds.has(String(item?.evaluator_id))) as CaseRecord[];
  }, [record]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (!focusedPointer) return;
    window.requestAnimationFrame(() => drawer.current?.querySelector<HTMLElement>(`[title="Copy JSON Pointer ${CSS.escape(focusedPointer)}"]`)?.closest<HTMLElement>(".json-row, .unified-row")?.scrollIntoView({ block: "center" }));
  }, [focusedPointer, tab, viewMode]);
  return (
    <div className="drawer-backdrop" role="presentation" onMouseDown={close}>
      <aside ref={drawer} tabIndex={-1} className="case-drawer" role="dialog" aria-modal="true" aria-label={`Case ${record.case?.id}`} onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="drawer-content">
          {tab === "Comparison" && <><div className="case-compact-summary"><div><small>TRANSITION</small><strong>{transition.label}</strong></div><div><small>PRIMARY FINDINGS</small><strong>{failingPointers.size}</strong></div><div><small>BASELINE</small><strong>{primaryOutcomeState(record.baseline_evaluation).label}</strong></div><div><small>CANDIDATE</small><strong>{primaryOutcomeState(record.candidate_evaluation).label}</strong></div></div>{diagnosticErrors.length > 0 && <InlineNotice tone="warning" title="Primary outcome unchanged by diagnostic errors">{diagnosticErrors.map((item) => item.evaluator_id).join(", ")} reported diagnostic errors. They are shown in Rules but do not override the selected primary outcome.</InlineNotice>}<div className="diff-toolbar"><span>{failingPointers.size} primary {failingPointers.size === 1 ? "finding" : "findings"}</span><button disabled={!failingPointers.size} onClick={() => drawer.current?.querySelector<HTMLElement>(".json-row-failed, .unified-row.json-row-failed")?.scrollIntoView({ block: "center" })}>Focus primary findings</button><button onClick={() => void navigator.clipboard.writeText(exactJsonStringify(record))}><Copy size={13} /> Copy exact evidence</button><div className="diff-mode" role="group" aria-label="Diff layout"><button className={viewMode === "side" ? "active" : ""} onClick={() => setViewMode("side")}><GitCompareArrows size={13} /> Side by side</button><button className={viewMode === "unified" ? "active" : ""} onClick={() => setViewMode("unified")}><PanelLeftClose size={13} /> Unified</button></div><label><input type="checkbox" checked={changedOnly} onChange={(event) => setChangedOnly(event.target.checked)} /> Collapse unchanged</label></div>{viewMode === "side" ? <SynchronizedComparison expected={record.case?.expected} baseline={output("baseline")} candidate={output("candidate")} baselineEvaluation={record.baseline_evaluation} candidateEvaluation={record.candidate_evaluation} highlights={failingPointers} changedOnly={changedOnly} /> : <UnifiedDiff expected={record.case?.expected} baseline={output("baseline")} candidate={output("candidate")} highlights={failingPointers} changedOnly={changedOnly} />}</>}
...
function SynchronizedComparison({ expected, baseline, candidate, baselineEvaluation, candidateEvaluation, highlights, changedOnly }: { expected: unknown; baseline: unknown; candidate: unknown; baselineEvaluation?: CaseRecord; candidateEvaluation?: CaseRecord; highlights: Set<string>; changedOnly: boolean }) {
  const syncing = useRef(false);
  const synchronize = (source: HTMLElement) => {
    if (syncing.current) return;
    syncing.current = true;
    const container = source.closest(".comparison-columns");
    container?.querySelectorAll<HTMLElement>(".json-tree").forEach((tree) => {
      if (tree !== source) {
        tree.scrollTop = source.scrollTop;
        tree.scrollLeft = source.scrollLeft;
      }
    });
    window.requestAnimationFrame(() => { syncing.current = false; });
  };
  return <div className="comparison-columns" onScrollCapture={(event) => synchronize(event.target as HTMLElement)}><JsonPanel title="Expected" value={expected} highlights={highlights} changedOnly={changedOnly} /><JsonPanel title="Baseline" value={baseline} expected={expected} evaluation={baselineEvaluation} highlights={highlights} changedOnly={changedOnly} /><JsonPanel title="Candidate" value={candidate} expected={expected} evaluation={candidateEvaluation} highlights={highlights} changedOnly={changedOnly} /></div>;
}

function JsonPanel({ title, value, expected, evaluation, highlights = new Set<string>(), changedOnly = false }: { title: string; value: unknown; expected?: unknown; evaluation?: CaseRecord; highlights?: Set<string>; changedOnly?: boolean }) {
  const rows = expected === undefined ? jsonRows(value).map((row) => ({ ...row, state: "reference" as DiffState })) : structuralDiff(expected, value);
  const state = evaluation ? primaryOutcomeState(evaluation) : null;
  return <section className="json-panel"><header><span><Braces size={15} />{title}</span>{state && <Status tone={state.kind === "pass" ? "pass" : state.kind === "fail" ? "fail" : state.kind === "error" ? "fail" : "neutral"} label={state.label} />}</header><div className="json-tree">{rows.filter((row) => !changedOnly || row.state !== "unchanged" || rowRelevant(row.path, highlights)).map((row) => <div key={row.path || "/"} className={`json-row diff-${row.state} ${[...highlights].some((pointer) => pointerMatches(pointer, row.path)) ? "json-row-failed" : ""}`} style={{ paddingLeft: `${10 + row.depth * 14}px` }}><i aria-label={diffLabel(row.state)}>{diffMark(row.state)}</i><button title={`Copy JSON Pointer ${row.path || "/"}`} onClick={() => void navigator.clipboard.writeText(row.path || "/")}><Copy size={12} /><code>{row.path || "/"}</code></button><span>{row.value}</span></div>)}</div></section>;
}
