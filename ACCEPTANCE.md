...
| Label isolation | ID pointers join all overlap checks; live adapters receive opaque tokens; Python callables and OpenAI templates receive no dataset ID; strict doctor fails expected-leaf and suspicious-ID leakage |
...
| Immutable finalized report | serving/export verifies every manifest-bound report asset and rejects unbound extras or symlinks; redaction fails closed and strips provider/retry echoes |
| Python adapter | synchronous and async callables work; exceptions are sanitized; malformed/nonserializable cases remain isolated without killing the worker |
...
| Evidence independence | explicit evidence-unit configuration excludes arbitrary operational metadata by default; repeated conflicts cannot choose the first row; row reordering leaves effect, interval, and gate unchanged |
