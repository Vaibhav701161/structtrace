...
function RunsPage() {
  ...
  return <div className="page page-wide"><PageHeader title="Comparisons" description="Completed evidence is retained with an explicit integrity state and survives a local server restart." />{runs.data?.length ? <Card className="history-table"><div className="history-head"><span>Comparison</span><span>Decision</span><span>Deployment change</span><span>Independent cases</span><span /></div>{runs.data.map((run) => { const gate = run.summary.gate; const qualityFailed = gate.quality_failures.length > 0; const verified = run.integrity.status === "verified"; const label = !verified ? "Evidence modified" : gate.deployment_authorized ? "Release authorized" : qualityFailed ? "Do not deploy" : gate.status === "insufficient_evidence" ? "Not enough evidence" : gate.status === "failed" ? "Do not deploy" : gate.gate_mode === "regression" ? "Regression passed" : "Analysis complete"; const tone = !verified || qualityFailed || gate.status === "failed" ? "fail" : gate.status === "insufficient_evidence" ? "warning" : gate.deployment_authorized ? "pass" : "info"; return <button className="history-row" key={run.runId} onClick={() => void navigate({ to: "/runs/$runId", params: { runId: run.runId } })}><span><strong>{run.projectName}</strong><small>{run.runId} · {verified ? "verified" : run.integrity.status}</small></span><Status tone={tone} label={label} /><code>{run.summary.paired.difference_pp >= 0 ? "+" : ""}{run.summary.paired.difference_pp.toFixed(1)} pp</code><strong>{run.summary.evidence.effective_inference_units}</strong><ArrowRight size={16} /></button>; })}</Card> : <Card><EmptyState icon={History} title={runs.isLoading ? "Loading comparisons…" : "No production comparisons yet"} description="Complete a recorded-output comparison and its decision will appear here." /></Card>}</div>;
}
