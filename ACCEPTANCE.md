...
| Local browser product | `structtrace open` serves an embedded offline UI through a random loopback capability; direct result refresh, typed assets, the visual invoice demo, a separately mapped recorded-output comparison, and a 100-case authorizing release lifecycle complete through the real engine. The authorized candidate is hash-bound as the next baseline under the same project while the prior run remains immutable. |
| Local UI security | Missing capabilities return 404, foreign Host returns 421, API responses are no-store with CSP/frame/referrer/MIME protections, browser requests carry bounded content rather than arbitrary file paths, source bytes are staged once and hash-checked, draft requests contain references rather than contents, and inactivity never stops an active run |
| Local UI accessibility | TypeScript and component tests pass; Chromium and Firefox Playwright plus axe cover first launch, the real invoice decision, dark mode, command search, responsive navigation, direct refresh, project reuse, and authorized baseline promotion. WebKit is mandatory in CI; this local host lacks its GTK/GStreamer libraries and cannot install them without administrator access. |
...
| Recorded workflow scale | Clean commit `0669ae9` completed the deterministic 10,000-case release-binary run in 135.54 s at 326,584 KiB peak RSS and complete replay in 2.12 s at 288,544 KiB; exact inputs, commands, artifact size, and digests are in `benchmarks/recorded-output-10000/result.json`, while the 100,000-case hard ceiling remains explicitly unmeasured |
